-- Separate the "returned for correction" workflow from temporary overrides.
-- District users may reset directorate_status from RETURNED to PENDING only
-- while resubmitting their own corrected record.
...
begin
  ...
  if current_role = 'district_user' then
    ...
    if new.status is distinct from old.status
       and new.status not in ('DRAFT', 'SUBMITTED_LOCKED', 'RETURNED') then
      raise exception 'District user cannot set this submission status';
    end if;

    if new.directorate_status is distinct from old.directorate_status then
      if not (
        old.directorate_status = 'RETURNED'
        and new.directorate_status = 'PENDING'
        and old.status = 'RETURNED'
        and new.status = 'SUBMITTED_LOCKED'
      ) then
        raise exception 'District user cannot change directorate review status';
      end if;
    end if;
